...
public class OrbitalAutoTest extends LinearOpMode {
    ...
    public void forward(int amt) {
        ...
        double adjPower = getPower(prop, 0.25, 1);
    ...
    public static double getPower(double amtDone, double edge, double max) {

        //Determine power based on an adjustable curve metric inspired by the Normal Distribution
        if (amtDone >= edge && amtDone <= 1 - edge) {
            ...
            double amtNormDone = amtDone / edge;
            return evaluateNormal(1, 0.333, amtNormDone, max);
    ...
    public static double evaluateNormal(double mu, double sigma, double x, double max) {
